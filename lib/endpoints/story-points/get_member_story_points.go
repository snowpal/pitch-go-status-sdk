...
func GetMemberStoryPoints(jwtToken string, memberId string) ([]response.StoryPoint, error) {
	var resStoryPoints response.StoryPoints

	route, err := helpers.GetRoute(lib.RouteStoryPointsGetMemberStoryPoints, memberId)
	if err != nil {
		fmt.Println(err)
